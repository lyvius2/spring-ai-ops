...
class GrafanaServiceTest {
    ...
    fun getAnalyzeFiringRecords_returnsList_whenRedisHasRecords() {
        ...
        `when`(zSetOperations.reverseRange("${REDIS_KEY_FIRING_PREFIX}my-app", 0, -1)).thenReturn(linkedSetOf(json))
    ...
    fun getAnalyzeFiringRecords_returnsEmptyList_whenRedisReturnsNull() {
        // given
        `when`(redisTemplate.opsForZSet()).thenReturn(zSetOperations)
        `when`(zSetOperations.reverseRange("${REDIS_KEY_FIRING_PREFIX}my-app", 0, -1)).thenReturn(null)
    ...
    fun getAnalyzeFiringRecords_filtersOutInvalidItems_whenDeserializationFails() {
        ...
        `when`(redisTemplate.opsForZSet()).thenReturn(zSetOperations)
        `when`(zSetOperations.reverseRange("${REDIS_KEY_FIRING_PREFIX}my-app", 0, -1)).thenReturn(linkedSetOf(validJson, invalidJson))
    ...
    @Test
    @DisplayName("'firing:{application}' key로 ZSet reverseRange 조회")
    fun getAnalyzeFiringRecords_queriesRedisWithCorrectKeyAndRange() {
        // given
        `when`(redisTemplate.opsForZSet()).thenReturn(zSetOperations)
        `when`(zSetOperations.reverseRange("${REDIS_KEY_FIRING_PREFIX}payment-service", 0, -1)).thenReturn(emptySet())

        // when
        grafanaService.getAnalyzeFiringRecords("payment-service")

        // then
        verify(zSetOperations).reverseRange("${REDIS_KEY_FIRING_PREFIX}payment-service", 0, -1)
    }
}

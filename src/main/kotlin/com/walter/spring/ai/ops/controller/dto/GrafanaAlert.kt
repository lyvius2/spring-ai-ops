...
data class GrafanaAlert(
    ...
    fun prometheusStartSeconds(bufferMinutes: Long = 5): String {
        return java.time.Instant.parse(startsAt)
            .minusSeconds(bufferMinutes * 60)
            .epochSecond.toString()
    }

    fun prometheusEndSeconds(): String {
        val instant = if (endsAt.startsWith("0001-")) {
            java.time.Instant.now()
        } else {
            java.time.Instant.parse(endsAt)
    ...
    fun prometheusLabels(): Map<String, String> = labels.filterKeys { it in PROMETHEUS_LABEL_KEYS }
    ...
    companion object {
        ...
        private val PROMETHEUS_LABEL_KEYS = setOf(
            "job", "instance", "namespace", "pod", "container",
            "app", "cluster", "env", "environment", "application",
            "service"
        )
    }
}

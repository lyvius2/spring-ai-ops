...
AI 기반 운영 자동화 도구로, **Grafana Alerting**, **GitHub**, **GitLab** 웹훅을 수신하여 LLM(OpenAI, Anthropic)으로 오류 분석, 코드 리뷰, 정적 코드 위험 분석을 실시간으로 수행합니다. Prometheus 메트릭 데이터를 선택적으로 수집하여 Loki 로그와 함께 LLM에 전달함으로써 더욱 풍부한 장애 원인 분석을 제공합니다.
...
3. **인시던트 인텔리전스** — Grafana 알림이 발생하면 Loki 로그를 자동으로 조회하고, Prometheus 메트릭(설정된 경우)을 함께 가져와 LLM에 전달합니다. 로그와 메트릭을 결합한 알림 컨텍스트를 분석하여 근본 원인 분석 결과를 대시보드에 실시간으로 스트리밍합니다.
...
| **LLM 장애 분석** | Grafana 알림 컨텍스트 + Loki 로그 + Prometheus 메트릭(선택) → 근본 원인, 영향 범위, 조치 방법 |
...
- **Connector** — Loki, GitHub, GitLab REST API를 위한 OpenFeign 클라이언트입니다.
...
POST /webhook/grafana[/{application}]
        ...
        │
        ├─ Loki 로그 조회  (loki.url이 설정된 경우)
        │    GET {loki.url}/loki/api/v1/query_range
        │
        ├─ Prometheus 메트릭 조회  ── 선택사항 (prometheus.url이 설정된 경우)
        │    GET {prometheus.url}/api/v1/query_range
        │    알림 레이블에서 메트릭 셀렉터 도출
        │    예: {job="my-app"} → CPU, 메모리, HTTP 에러율 등
        │
        ├─ LLM 분석 요청
        │    알림 컨텍스트
        │    + Loki 로그 라인   (있는 경우)
...
- (선택) Loki 연동 시 Loki 서버 URL
- (선택) Prometheus 메트릭 조회 시 Prometheus 서버 URL
...
> Prometheus 레이블(`job`, `instance` 등)과 Loki 스트림 레이블이 동일해야 메트릭·로그 조회가 자동으로 동작합니다. Promtail 또는 Grafana Alloy 설정을 확인하세요.
